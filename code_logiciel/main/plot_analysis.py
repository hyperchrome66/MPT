...
def plot_msd(data):
    ...
    for i in range(len(data)):
        ...
        def plot_msd_filtered(data, alpha_min = 0.75):
...
def plot_alpha_multiple(data):
    ...
    for i in range(len(data)):
        alpha_values = np.array(data[i]["alpha2"])
        alpha_values = alpha_values[alpha_values >= 0] #filtre : alphas < 0 éliminées 
    ...
    # Calcul global pour gérer l'espace au-dessus
    global_max = max([max(a) for a in all_alpha])
    global_min = min([min(a) for a in all_alpha])
    ...
    for j, alpha_values in enumerate(all_alpha):
        median_alpha = np.median(alpha_values) #calcul pour chaque fichier de la médiane 
    ...
    plt.ylabel("Alpha")
    plt.title("Distribution de alpha")
    plt.xlabel("")
